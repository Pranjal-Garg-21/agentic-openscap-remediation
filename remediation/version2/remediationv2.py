...
def load_keep_rules(role, profile):
    wb = load_workbook(resolve_path(GROUND_TRUTH_XLSX), data_only=True)
...
def save_results(all_results, role, profile):
    ...
    run_dir  = os.path.join(resolve_path(RESULTS_DIR), f"remediation_{slug}_{ts}")
...
def main():
    ...
    for f in [SCAN_RESULT_XML, GROUND_TRUTH_XLSX]:
        if not os.path.exists(resolve_path(f)):
            print(f"\n[ERROR] Missing: {f}")
            sys.exit(1)
