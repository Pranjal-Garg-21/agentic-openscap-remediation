...
RULE_TIMEOUTS = {
    ...
    "grub2_uefi_password": 180,   # update-grub can be slow; was timing out on the interactive hang before, this is unrelated headroom
}
...
if dpkg-query --show --showformat='${db:Status-Status}\\n' 'nftables' 2>/dev/null | grep -q '^installed'; then
    ...
    "accounts_tmout": """\
# The check scans ALL .sh files in /etc/profile.d and requires every TMOUT
# occurrence anywhere to be <=900 -- clean up any stale/conflicting file
# from earlier attempts before writing the correct one.
...
# These three exact lines, in this relative order, are what the compliance
# check itself requires (confirmed from the actual OVAL check regex, not a
# guess) -- pam-auth-update was proving unreliable at actually wiring these
# in, so write them directly instead of depending on it.
if ! grep -qE '^[[:space:]]*auth[[:space:]]+(requisite|required)[[:space:]]+pam_faillock\\.so.*preauth' "$AUTH_FILE"; then
    sed -i '1i auth        requisite                       pam_faillock.so preauth silent' "$AUTH_FILE"
fi

if ! grep -qE '^[[:space:]]*auth[[:space:]]+\\[default=die\\][[:space:]]+pam_faillock\\.so[[:space:]]+authfail' "$AUTH_FILE"; then
    echo 'auth        [default=die]                   pam_faillock.so authfail' >> "$AUTH_FILE"
fi

if ! grep -qE '^[[:space:]]*account[[:space:]]+required[[:space:]]+pam_faillock\\.so[[:space:]]*(#.*)?$' "$ACCOUNT_FILE"; then
    echo 'account     required                        pam_faillock.so' >> "$ACCOUNT_FILE"
fi""",
    "accounts_password_pam_minlen": """\
# The check tolerates leading whitespace before the key AND requires EVERY
# matching line in the file to individually satisfy the value -- fix all
# occurrences (including indented ones), not just a bare no-indent line.
...
touch "$conf_file"
sed -i -E '/^(ServerKeyFile|ServerCertificateFile|TrustedCertificateFile)[[:blank:]]/d' "$conf_file"
cat >> "$conf_file" << 'EOF'
ServerKeyFile=/etc/pki/systemd/private/journal-upload.pem
ServerCertificateFile=/etc/pki/systemd/certs/journal-upload.pem
TrustedCertificateFile=/etc/pki/systemd/ca/trusted.pem
EOF""",
    "systemd_journal_upload_url": """\
# The check accepts EITHER /etc/systemd/journal-upload.conf OR a file
# under journal-upload.conf.d/*.conf, but wants EXACTLY ONE uncommented
# URL= line total across both -- comment out any in the base file first,
# so a leftover from an earlier attempt doesn't create a second instance.
# IMPORTANT: match "URL=value" (no space before =) too, not just "URL value"
# -- a prior version of this fix required a space/tab right after URL and
# silently never matched lines like "URL=http://127.0.0.1:19532", which is
# exactly the format systemd's own default ships with.
if [ -f /etc/systemd/journal-upload.conf ]; then
    sed -i -E 's/^([[:space:]]*URL[[:space:]]*=.*)/#\\1/' /etc/systemd/journal-upload.conf
fi
mkdir -p /etc/systemd/journal-upload.conf.d
conf_file=/etc/systemd/journal-upload.conf.d/60-journald_upload.conf
touch "$conf_file"
sed -i -E '/^[[:space:]]*URL[[:space:]]*=/d' "$conf_file"
echo 'URL=remotelogserver' >> "$conf_file"
""",
    "file_groupowner_backup_etc_gshadow": """\
# Confirmed from the check itself: it wants group 0 (root), NOT the
# 'shadow' group -- the earlier fix used the wrong group entirely.
chgrp 0 /etc/gshadow-
ls -l /etc/gshadow-
""",
    "grub2_uefi_password": """\
...
# IMPORTANT: /etc/grub.d/40_custom is an executable script that update-grub
# RUNS -- its stdout becomes the actual grub.cfg content. Without this
# shebang + "exec tail -n +3 $0" header (the standard Ubuntu template),
# update-grub tries to literally EXECUTE the lines below as shell commands,
# and fails with "password_pbkdf2: not found" since that's not a real
# program. This header makes everything after line 3 print as output
# instead of running as commands.
...
""",
    "partition_for_tmp": """\
# Confirmed from the actual OVAL check: it only verifies /tmp is its own
# LIVE mount point right now (linux:partition_object on /proc/mounts) --
# it does not require a real disk partition and does not even check
# /etc/fstab. A tmpfs mount fully satisfies it.
...
RULE_HINTS = {
    "sudo_require_reauthentication":
        ...
        "The confirmed required value on this exact host's SCAP profile is "
        "15 (minutes) -- NOT 0 and NOT -1. A negative value like -1 means "
        "the opposite of what's wanted: per the sudo manual, a value less "
        "than 0 makes the cached credential NEVER expire. A value of 0 "
        "would force a password every single command, which is stricter "
        "than this profile actually requires and will fail the check "
        "(and will also break every other sudo call on this host, "
        "including this pipeline's own internal ones, if applied). Use "
        "exactly 15. ALSO: any file you create under /etc/sudoers.d/ must "
        "be mode 0440 (owner and group read-only, no write, no world "
        "access) -- `visudo -c` rejects any sudoers.d file with different "
        "permissions. Always `chmod 0440` the file you write, in the same "
    ...
    "accounts_password_pam_minlen":
        "Set this in /etc/security/pwquality.conf (a `minlen=14` line -- "
        "confirmed from this exact host's SCAP profile, not a guess), "
    ...
    "accounts_tmout":
        ...
        "readonly TMOUT\n"
        "export TMOUT\n"
        "(900 is confirmed from this exact host's SCAP profile.) The "
        "compliance check almost certainly greps the FILE for all three "
        "lines, not the live shell state during remediation.",
    ...
    "accounts_passwords_pam_faillock_deny":
        "Use `deny = 4` -- confirmed from this exact host's SCAP profile, "
        "not a guess. Append to /etc/security/faillock.conf with `>>`, not "
        "overwrite with `>` -- other faillock rules write to this same "
        "file.",
    "accounts_passwords_pam_faillock_unlock_time":
        "Use `unlock_time = 900` -- confirmed from this exact host's SCAP "
        "profile, not a guess. Append to /etc/security/faillock.conf with "
        "`>>`, not overwrite with `>`.",
    "accounts_passwords_pam_faillock_enabled":
        "CONFIRMED from the actual OVAL check on this host (not a guess): "
        "the check requires these exact three lines to exist, in this "
        "relative order. In /etc/pam.d/common-auth: (1) a line matching "
        "`auth requisite|required pam_faillock.so ... preauth ...` "
        "BEFORE the existing pam_unix.so line, and (2) a line matching "
        "`auth [default=die] pam_faillock.so authfail` AFTER the "
        "pam_unix.so line. In /etc/pam.d/common-account: (3) a line "
        "matching `account required pam_faillock.so`. Using "
        "`pam-auth-update` to add these via /usr/share/pam-configs/ has "
        "proven unreliable in practice on this host -- write these three "
        "lines DIRECTLY into common-auth/common-account with sed/grep "
        "guards (insert the preauth line at the very top of common-auth, "
        "append the authfail line at the end, append the account line to "
        "common-account) instead of depending on pam-auth-update to apply "
        "a custom profile.",
    "systemd_journal_upload_server_tls":
        "Don't pass a regex pattern like `[^/]+\\.conf` directly to grep as "
        "if it were a real filename -- use an actual path or a shell glob "
        "loop. Also, and more importantly: commenting out old settings "
        "isn't enough on its own -- you must write the actual required "
        "values, confirmed from this exact host's SCAP profile: "
        "ServerKeyFile=/etc/pki/systemd/private/journal-upload.pem, "
        "ServerCertificateFile=/etc/pki/systemd/certs/journal-upload.pem, "
        "TrustedCertificateFile=/etc/pki/systemd/ca/trusted.pem.",
    "systemd_journal_upload_url":
        "Don't pass a regex pattern directly to grep as if it were a real "
        "filename -- use an actual path or a shell glob loop. Also, and "
        "more importantly: commenting out old settings isn't enough on its "
        "own -- you must write the actual required value, confirmed from "
        "this exact host's SCAP profile: URL=remotelogserver (this is a "
        "literal hostname value in the profile, not a placeholder to "
        "replace).",
...
Ground-truth expert assessment for this profile:
  This compliance RULE is: {gt_decision} for this profile
  (KEEP means this rule's requirement SHOULD be enforced/remediated on this
  host -- it does NOT mean 'keep' whatever package/setting/file the rule
  happens to be about. E.g. for a rule titled "remove package X", a KEEP
  decision means you SHOULD remove package X. SKIP would mean this rule
  doesn't apply here at all, but you're only seeing this rule because it's
  KEEP -- so proceed with the rule's actual technical requirement as
  described in the Title/Description below.)
  Reason for this decision: {gt_reason}
(This rule is only sent to you because the ground-truth decision for this
profile is KEEP -- write the correct technical remediation script that
satisfies the rule's actual requirement.)
...
def verify_rule(rule_id):
    cmd = (
        ...
        f"--profile xccdf_org.ssgproject.content_profile_cis_level1_workstation "
...
# Rules that, once they actually take effect, can lock you out of the VM's
# normal login/sudo path -- these warn and require explicit confirmation
# EVEN IN --auto MODE, since a lockout is a much bigger problem than a
# skipped auto-approval. Confirmed real: accounts_passwords_pam_faillock_*
# genuinely locked the account out after this many test passes tripped its
# own counter once wired in.
LOCKOUT_RISK_RULES = {
    "accounts_passwords_pam_faillock_enabled":
        "This wires pam_faillock into your actual login path for the first "
        "time. Once active, mistyping your password a few times in a row "
        "(the deny count) WILL lock the account out for real -- this has "
        "already happened once during this project.",
    "accounts_passwords_pam_faillock_deny":
        "Sets how many wrong passwords trigger a lockout. If faillock is "
        "already enabled, a low value here makes an accidental lockout "
        "easier to trigger.",
    "accounts_passwords_pam_faillock_unlock_time":
        "Sets how long a lockout lasts once triggered (currently 15 min). "
        "Doesn't cause a lockout by itself, but determines how long you're "
        "stuck if one happens.",
    "sudo_require_reauthentication":
        "Changes how long sudo caches your password. A wrong value here "
        "(this has happened before -- an earlier wrong guess of '0' broke "
        "sudo caching entirely and made every sudo call demand a fresh "
        "password) can make the whole pipeline unusable, though it won't "
        "lock your login itself.",
    "sudo_remove_no_authenticate":
        "Edits /etc/sudoers and /etc/sudoers.d/* directly (commenting out "
        "!authenticate lines). A bad edit here can break sudo entirely, "
        "same family of risk as the sudoers corruption seen earlier in "
        "this project.",
}


def warn_lockout_risk(short):
    """Blocks, even under --auto, before running a rule that has a real
    history of locking this VM out. Returns True if the user wants to
    proceed."""
    ...
    print(f"    Before continuing: make sure you have a SECOND terminal/SSH "
          f"session already logged in to this VM, in case this one locks "
          f"you out.")
    print(f"    {'='*60}")
    ans = input("    Proceed with this rule? [y/n]: ").strip().lower()
    return ans == "y"
...
def remediate_rule(model, rule_id, rule_info, gt_row, decision, profile_key,
    ...
    while pass_num < MAX_PASSES:
        ...
        # For rules with a pre-verified script, don't trust the model to
        # faithfully copy it from the prompt -- it doesn't always (confirmed:
        # grub2_uefi_password kept reproducing its own broken pattern even
        # after being handed the exact working script and told to use it
        # as-is). Apply the known-good script directly on pass 1 instead.
        if pass_num == 1 and not clarification and short in KNOWN_FIXES:
            print(f"    Using pre-verified script for {short} (skipping LLM "
                  f"query -- known fixes aren't reliably reproduced verbatim "
                  f"by the model).")
